...
class VAEattnRNN(ModelCore):
	...
	def build(self, for_deploy, variants=""):
		...
		KLDs = 0.0
		z, KLD, l2 = CreateVAE([self.enc_states[-1]], self.conf.enc_latent_dim, name_scope="VAE")
		KLDs += KLD
		final_enc_states = [self.enc_states[-1].h] 
		...
		with tf.name_scope("DynRNNDecode") as scope:
			...
			cell = AttnCell(cell_model=conf.cell_model, num_units=mem_size, num_layers=conf.num_layers,
							...
							max_mem_size=max_mem_size, addmem=self.conf.addmem, z=beam_z, keep_prob=conf.keep_prob,
							dtype=tf.float32, name_scope="AttnCell")

			zero_attn_states = DecStateInit(all_enc_states=beam_init_states, decoder_cell=cell, batch_size=batch_size * self.beam_size, init_type="allzeros", use_proj=False)
		...
		if not for_deploy:	
			...
			with tf.name_scope("Loss") as scope:
				...
				self.loss = tf.reduce_sum((example_losses + self.conf.kld_ratio * KLDs) * self.down_wgts) / batch_wgt
