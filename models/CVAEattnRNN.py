...
class CVAEattnRNN(ModelCore):
	...
	def build(self, inputs, for_deploy):
		...
		z, mu_prior, logvar_prior = PriorNet([enc_state], hidden_units, self.conf.enc_latent_dim, stddev=1, prior_type=conf.prior_type)
		...
		cell = AttnCell(cell_model=conf.cell_model, num_units=mem_size, num_layers=conf.num_layers,
						...
						max_mem_size=max_mem_size, addmem=self.conf.addmem, z=beam_z, keep_prob=1.0,
						dtype=tf.float32, name_scope="AttnCell")
		# Fit decision states to shape of attention decoder cell states 
		zero_attn_states = DecStateInit(beam_raw_dec_states, cell, batch_size * self.beam_size, conf.dec_init_type, conf.use_init_proj)
		...
		if not for_deploy: 
			...
			with tf.name_scope("Loss") as scope:
				...
				self.loss = tf.reduce_sum(example_losses + self.conf.kld_ratio * KLD) / total_wgts 
