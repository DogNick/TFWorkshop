...
class DeepMatchInteract(object):
    ...
    def build(self):
        ...
        norm_q = tf.sqrt(tf.reduce_sum(tf.square(q_out_state), 1, keep_dims=True))
        norm_p = tf.sqrt(tf.reduce_sum(tf.square(p_out_state), 1, keep_dims=True))
        norm_r = tf.sqrt(tf.reduce_sum(tf.square(r_out_state), 1, keep_dims=True))
        norm_neg_r = tf.sqrt(tf.reduce_sum(tf.square(neg_r_out_state), 1, keep_dims=True))
        cos_qp = tf.reduce_sum(q_out_state * p_out_state, 1, keep_dims=True) / (norm_q * norm_p)
        cos_qr = tf.reduce_sum(q_out_state * r_out_state, 1, keep_dims=True) / (norm_q * norm_r)
        cos_qnegr = tf.reduce_sum(q_out_state * neg_r_out_state, 1, keep_dims=True) / (norm_q * norm_neg_r)
        ...
        qnegr_vec = tf.concat([tf.contrib.layers.flatten(qnegr_map), cos_qnegr], 1)
    ...
    def fetch_test_data(self, records, begin=0, size=1):
        ...
        for line in records:
            ...
            feed_data.append([q, len(q) + 1, p, len(p) + 1, r, len(r) + 1, [], 1])
            golds.append(segs[-1])
...
if __name__ == "__main__":
    with tf.device("/gpu:2"):
        model = DeepMatchInteract(name="stc-2-interact", for_deploy=False)
        model.build()

    init_ops = model.get_init_ops(job_type="single", task_id=0)

    path = os.path.join(confs["stc-2-interact"].data_dir, "train.data")
