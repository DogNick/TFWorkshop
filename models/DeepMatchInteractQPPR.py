...
class DeepMatchInteractQPPR(object):
    ...
    def build(self):
        ...
        self.neg_scores = tf.squeeze(qp_fc * pnegr_fc + qnegr_fc)
...
if __name__ == "__main__":
    with tf.device("/gpu:2"):
        model = DeepMatchInteractQPPR(name="stc-2-interact-qppr-2", for_deploy=False)
        model.build()

    init_ops = model.get_init_ops(job_type="single", task_id=0)

    path = os.path.join(confs["stc-2-interact-qppr-2"].data_dir, "train.data")
