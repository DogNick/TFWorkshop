...
class _BaseAttentionMechanism(AttentionMechanism):
  ...
  def __init__(self,
    ...
    with ops.name_scope(
      ...
      if max_mem_size: 
        self._alignments_size = max_mem_size
      else:
...
class LuongAttention(_BaseAttentionMechanism):
  ...
  def __init__(self, num_units, memory, max_mem_size, memory_sequence_length=None,
               scale=False, probability_fn=None, z=None,
               name="LuongAttention"):
  ...
  def __call__(self, query, previous_alignments, newmem=None):
...
class BahdanauAttention(_BaseAttentionMechanism):
  ...
  def __init__(self, num_units, memory, max_mem_size, memory_sequence_length=None,
               normalize=False, probability_fn=None, z=None,
...
class AttentionWrapper(rnn_cell_impl.RNNCell):
  ...
  def state_shape(self):
    alignment_history_shape = tf.TensorShape([]) if self._alignment_history else ()
    return AttentionWrapperState(
            cell_state=nest.map_structure(lambda s: tf.TensorShape([None, s]), self._cell.state_size),
                time=tensor_shape.TensorShape([]),
                attention=tf.TensorShape([None, self._attention_layer_size]),
                alignments=tf.TensorShape([None, 1, None]),
                alignment_history=alignment_history_shape,# alignment_history is sometimes a TensorArray
  ...
  def state_size(self):
    return AttentionWrapperState(
				...
				newmem=self._attention_layer_size)  
  ...
  def zero_state(self, batch_size, dtype):
    with ops.name_scope(type(self).__name__ + "ZeroState", values=[batch_size]):
      ...
      zero_newmem = zero_newmem.write(0, tf.zeros([batch_size, self._attention_layer_size]))
      return AttentionWrapperState(
          cell_state=cell_state,
          time=array_ops.zeros([], dtype=dtypes.int32),
          attention=_zero_state_tensors(
          self._attention_layer_size, batch_size, dtype),
          alignments=tf.expand_dims(self._attention_mechanism.initial_alignments(batch_size, dtype), 1),
  ...
  def call(self, inputs, state, scope=None):
    ...
    cell_output, next_cell_state = self._cell(cell_inputs, cell_state, scope=scope)
    cell_batch_size = (
    ...
    newmem_stack = tf.transpose(state.newmem.stack(), [1, 0, 2])
    if self._addmem:
        alignments = self._attention_mechanism(state_to_query, state.alignments, newmem_stack)
    else:
        alignments = self._attention_mechanism(state_to_query, state.alignments)
    ...
    if self._addmem:
        context = math_ops.matmul(expanded_alignments, tf.concat([self._attention_mechanism.values, newmem_stack], 1))
    else:
    ...
    batch_size = tf.shape(expanded_alignments)[0]
    pad_size = self._attention_mechanism._max_mem_size - tf.shape(expanded_alignments)[2]
    alignments = tf.concat([expanded_alignments, tf.zeros([batch_size, 1, pad_size])], 2)
