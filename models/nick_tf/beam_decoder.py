...
import decoder1_2 as decoder
import helper1_2 as helper_py
import dynamic_attention_wrapper, attention_wrapper1_2 
...
class BeamDecoder(decoder.Decoder):
  def __init__(self, cell, helper, out_proj, initial_state=None, beam_splits=[1], max_res_num=10,  output_layer=None):
  ...
  def state_shape(self):
    if isinstance(self._cell, DynamicAttentionWrapper) or isinstance(self._cell, AttentionWrapper):
  ...
  def initialize(self, name=None):
    ...
    if self._initial_state == None:
        attn_cell_zeros = self._cell.zero_state(self.batch_size, tf.float32)
        self._initial_state = BeamState(tf.zeros([self.batch_size]), attn_cell_zeros, tf.zeros([self.batch_size], tf.int32))
  ...
  def step(self, time, inputs, state, name=None):
    ...
    with tf.variable_scope(name, "BasicDecoderStep") as scope:
      cell_outputs, cell_state = self._cell(inputs, attn_state, scope=scope)
    ...
    def f2():
      ...
      # this batch_size = batch_size_origin * beam_size
      offset = tf.reshape(tf.ones([self.batch_size], dtype=tf.int32) * self._beam_size, [-1, self._beam_size])
    ...
    if isinstance(self._cell, DynamicAttentionWrapper) or isinstance(self._cell, AttentionWrapper):
      ...
      if isinstance(self._cell, DynamicAttentionWrapper):
          ...
          beam_states = AttentionWrapperState(tuple(next_states), next_attns, time, alignments, (), newmem)
    else:
