...
class DeepMatchInteractConcatRNN(object):
    ...
    def build(self):
        ...
        with variable_scope.variable_scope("r_rnn", dtype=dtype, reuse=True) as scope: 
            ...
            neg_r_out, neg_r_out_state = bidirectional_dynamic_rnn(cell_fw=cell1, cell_bw=cell2,
                                                           ...
                                                           time_major=False)
        fw, bw = q_out_state
        q_out_state = tf.concat([fw[0].h, bw[0].h], axis=1)
        fw, bw = p_out_state
        p_out_state = tf.concat([fw[0].h, bw[0].h], axis=1)
        fw, bw = r_out_state
        r_out_state = tf.concat([fw[0].h, bw[0].h], axis=1)
        fw, bw = neg_r_out_state
        neg_r_out_state = tf.concat([fw[0].h, bw[0].h], axis=1)
...
if __name__ == "__main__":
    with tf.device("/gpu:2"):
        model = DeepMatchInteractConcatRNN(name="stc-2-interact-concat-rnn", for_deploy=False)
        model.build()

    init_ops = model.get_init_ops(job_type="single", task_id=0)

    path = os.path.join(confs["stc-2-interact-concat-rnn"].data_dir, "train.data")
