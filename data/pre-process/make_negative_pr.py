...
for i in range(len(idx)):
    ...
    for j in range(begin, end):
        ...
        if random.random() > m * 1.0 / ((m + n) * 1.0):
            p_idx = random.sample(xrange(end, len(data)), N)
        else:
            p_idx = random.sample(xrange(0, begin), N)

        # q, q r,q- r-
        for r_i in r_idx:
            q, p, r, neg_p, neg_r = data[j][0].strip(), data[j][0].strip(), data[j][1].strip(), data[r_i][0], data[r_i][1] 
            if q == "" or p == "" or r == "" or neg_r == "":
                print "DEP:", q, p, r, neg_p, neg_r
                continue
            fout.write("%s\n" % "\t".join([q, p, r, neg_p, neg_r]))

        # q, q* r, q- r-
        for q_i in q_idx:
            q, p, r, neg_p, neg_r = data[j][0].strip(), data[q_i][0].strip(), data[j][1].strip(), data[r_i][0], data[r_i][1] 
            if q == "" or p == "" or r == "" or neg_r == "":
                print "DEP:", q, p, r, neg_p, neg_r
                continue
            fout.write("%s\n" % "\t".join([q, p, r, neg_p, neg_r]))
        ...
        # q, q* r, q*- r-
        for q_i, p_i in (q_idx, p_idx):
            q, p, r, neg_p, neg_r = data[j][0].strip(), data[p_i][0].strip(), data[j][1].strip(), data[p_i][0], data[r_i][1] 
            if q == "" or p == "" or r == "" or neg_r == "":
                print "DEP:", q, p, r, neg_p, neg_r
                continue
            fout.write("%s\n" % "\t".join([q, p, r, neg_p, neg_r]))
